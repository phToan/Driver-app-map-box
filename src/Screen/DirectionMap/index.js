...
const DirectionMap = ({ navigation }) => {
  ...
  const fetchDirectionData = async () => {
    try {
      ...
      console.log(data.routes[index]?.geometry?.coordinates);
  ...
  const updateCoordinate = (index) => {
    const dataRef = ref(database, `active_status/${key}`);
    update(dataRef, {
      lat: coordinates[index][1],
      long: coordinates[index][0],
      // routing: '',
    }).catch((err) => {
      console.log('update coordinate error: ', err);
    })
  }
  ...
  const onStart = () => {
    if (!start) {
      ...
      setStart(false);
      setEndDirection(true);
      updateCoordinate(coordinates.length - 1)
    }
  };
